...
def get_clip_score(tensor, words):
    ...
    for i in range(tensor.shape[0]):
        ...
        img_resize = transforms.Resize((64, 64))
...
class L_clip(nn.Module):
    ...
    def forward(self, pred_LL, denoise_LL_LL):
        prompt1 = ["A vivid image with a clear background and visible objects ",
                   "It's a target salient, semantically informative image "]
        prompt2 = ["It's a regular picture"]
...
def get_clip_score_MSE(pred, inp, weight):
    ...
    for i in range(pred.shape[0]):
        ...
        pred_img = clip_normalizer(pred_img.reshape(1, 3, 64, 64))
        ...
        for feature_index in range(len(weight)):
            MSE_loss_per_img = MSE_loss_per_img + weight[feature_index] * F.mse_loss(
                pred_image_features[1][feature_index].squeeze(0), inp_image_features[1][feature_index].squeeze(0))
